...
# from speechbrain.nnet.complex_networks import transformer
...
class CBAMLayer(nn.Module):        # 应用CBAM--------------------------------------------------------        # 应用CBAM--------------------------------------------------------
...
class ECABlock(nn.Module):   # 应用ECA通道注意力机制--------------------------------------------------------        # 应用ECA-------------------------------------------------
...
class ConvBlock(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.init_weight()
        # self.cbam = CBAMLayer(channel=out_channels, reduction=16, spatial_kernel=7)         # 应用ECA--------------------------------------------------------
...
class ViT(nn.Module):
    def __init__(self, *, image_size, patch_size, num_classes, dim, depth, heads, mlp_dim, pool='cls', channels=3, dim_head=64, dropout=0., emb_dropout=0.):
        ...
        self.pos_embedding = nn.Parameter(torch.randn(1, num_patches + 1, dim))  # 使用nn.Parameter 增加可学习参数
...
#
#         # self.cbam_block = CBAMLayer(channel=1, reduction=16, spatial_kernel=7)
#         # self.nonlocal_block =  NonLocalBlock(in_channels=1)
#
#         self.fc1 = nn.Linear(2048, 2048, bias=True)
#         self.fc_audioset = nn.Linear(2048, classes_num, bias=True)
#
#         self.transformer = Transformer(dim=64, depth=1, heads=2, dim_head=4, mlp_dim= 8)
#         # self.fc_fi = nn.Linear(601, 18, bias=True)
#         self.adaptive_pool = nn.AdaptiveAvgPool1d(18)
#
#         self.init_weight()
#
#     def init_weight(self):
...
#
#
#         # print('input shape:   ',input.shape)    #input.shape=[32，19200]
#         x = self.spectrogram_extractor(input)   # (batch_size, 1, time_steps, freq_bins)
#         #
#
#         print('x.spectrogram_extractor.shape = ',x.shape) #shape =  torch.Size([32, 1, 601, 513])
#         x = self.logmel_extractor(x)    # (batch_size, 1, time_steps, mel_bins)
#         print('x.logmel_extractor.shape = ',x.shape)   #shape =  torch.Size([32, 1, 601, 64])
#
#
#
#         x = x.transpose(1, 3)
#         # print('x.transpose.shape:      ',x.shape)  #shape =torch.Size([32, 64, 601, 1])
#         x = self.bn0(x)
#         # print('x.bn0.shape:      ', x.shape)    #  torch.Size([32, 64, 601, 1])
#         x = x.transpose(1, 3)
#         # print('x.transpose.shape:      ', x.shape)   #torch.Size([32, 1, 601, 64])
#
#         if self.training:
#             x = self.spec_augmenter(x)
#             # print('x.spec_augmenter.shape:      ', x.shape)
#
#         # Mixup on spectrogram
#         if self.training and mixup_lambda is not None:
#             x = do_mixup(x, mixup_lambda)
#             # print('x.do_mixup.shape:      ', x.shape)
#
#         a = x.squeeze()
#         # print('a.shape(x.squeeze) = ', a.shape)  # 32，601，64
#         a = self.transformer(a)
#         # print('transformer(a).shape = ',a.shape)  # 32，601，64
#
#
#         x = self.conv_block1(x, pool_size=(2, 2), pool_type='avg')
#         # print('x.conv_block1.shape:      ', x.shape)   # torch.Size([32, 64, 300, 32])
#         x = F.dropout(x, p=0.2, training=self.training)
#
#         x = self.conv_block2(x, pool_size=(2, 2), pool_type='avg')
#         # print('x.conv_block2.shape:      ', x.shape)    # torch.Size([32, 128, 150, 16])
#         x = F.dropout(x, p=0.2, training=self.training)
#
#         x = self.conv_block3(x, pool_size=(2, 2), pool_type='avg')
#         # print('x.conv_block3.shape:      ', x.shape)    # torch.Size([32, 256, 75, 8])
#         x = F.dropout(x, p=0.2, training=self.training)
#
#         x = self.conv_block4(x, pool_size=(2, 2), pool_type='avg')
#         # print('x.conv_block4.shape:      ', x.shape)   #  torch.Size([32, 512, 37, 4])
#         x = F.dropout(x, p=0.2, training=self.training)
#
#         x = self.conv_block5(x, pool_size=(2, 2), pool_type='avg')
#         # print('x.conv_block5.shape:      ', x.shape)   #  torch.Size([32, 1024, 18, 2])
#         x = F.dropout(x, p=0.2, training=self.training)
#
#         x = self.conv_block6(x, pool_size=(1, 1), pool_type='avg')
#         # print('x.conv_block6.shape:      ', x.shape)   # torch.Size([32, 2048, 18, 2])
#         x = F.dropout(x, p=0.2, training=self.training)
#         # x = self.transformer(x)
#         x = torch.mean(x, dim=3)
#         # print('x.mean.shape = ', x.shape)  #  torch.Size([32, 2048, 18])
#         # 使用插值调整序列长度
#         a_interpolated = F.interpolate(a, size=2048, mode='linear', align_corners=True)
#         # print('a_interpolated.shape: ',a_interpolated.shape)
#         #
#         # a_interpolated.shape = torch.Size([32, 601, 2048])
#         #
#         # a_transformed.shape = torch.Size([32, 601, 18])
#         a = a_interpolated.transpose(1,2)
#         # print('a.shape: ', a.shape)   #[32, 2048, 601]
#
#
#         # 使用池化层调整特征维度
#         a_transformed = self.adaptive_pool(a)
#         # print('a_transformed.shape = ',a_transformed.shape)   #[32, 2048, 18]
#         #
#         # print('x.shape:',x.shape)
#
#         x = (x + a_transformed) / 2   #[32, 2048, 18]
#
#         (x1, _) = torch.max(x, dim=2)
#         # print('x1.mean.shape = ', x1.shape)   #  torch.Size([32, 2048])
#         x2 = torch.mean(x, dim=2)
#         # print('x2.mean.shape = ', x2.shape)   #  torch.Size([32, 2048])
#         x = x1 + x2
#         x = F.dropout(x, p=0.5, training=self.training)
#         x = F.relu_(self.fc1(x))
#         # print('x.relu.shape = ', x.shape)    #  torch.Size([32, 2048])
#
#
...
#
#         # self.transformer = Transformer(dim=64, depth=1, heads=2, dim_head=4, mlp_dim=8)
#         # self.transformer = SpectAtt(dim=8, depth=1, heads=2, dim_head=4, mlp_dim=4, h=601, w=64, att_dropout = 0., ff_dropout = 0., drop_path = 0.)
#
#         # self.specformer = SpectFormer(
#         #      image_size=224,
#         #      patch_size=16,
#         #      num_classes=2,
#         #      dim=512,
