...
class AudioSetDataset(object):
    def __init__(self, sample_rate=32000, csv_path="/media/sam/TOSHIBA/dataset/langSplit6s/official_eval3_segments.csv" ,
                              classes_num= 2 , id_to_ix={}, audio_dir='/media/sam/TOSHIBA/dataset/langSplit6s/test3'):
        """This class takes the meta of an audio clip as input, and return 
        the waveform and target of the audio clip. This class is used by DataLoader. 
        """
        self.sample_rate = sample_rate
        self.meta_dict = read_metadata(csv_path, classes_num, id_to_ix)
    ...
    def __getitem__(self, meta):
        ...
        audio_name=self.meta_dict['audio_name'][meta['index_in_hdf5']].encode()
        audio_path = os.path.join(self.audios_dir, self.meta_dict['audio_name'][meta['index_in_hdf5']])
        (audio, _) = librosa.core.load(audio_path, sr=self.sample_rate, mono=True)

        audio = pad_or_truncate(audio, self.sample_rate * 6)
        waveform = float32_to_int16(audio)
        target = self.meta_dict['target'][meta['index_in_hdf5']]
        data_dict = {
