...
def train(args):
    ...
    train_indexes_hdf5_path = os.path.join(workspace, 'hdf5s', 'indexes', 
        'balanced_train_demucs.h5')

    eval_bal_indexes_hdf5_path = os.path.join(workspace, 
        'hdf5s', 'indexes', 'balanced_train_demucs.h5')

    eval_test_indexes_hdf5_path = os.path.join(workspace, 'hdf5s', 'indexes', 'eval3.h5')
    ...
    # dataset = AudioSetDataset(sample_rate=sample_rate, csv_path="/media/sam/TOSHIBA/dataset/langSplit6s/official_eval3_segments.csv" ,
    #                           classes_num= classes_num , id_to_ix= id_to_ix, audio_dir='/media/sam/TOSHIBA/dataset/langSplit6s/test3')
    ...
    for batch_data_dict in train_loader:
        ...
        if (iteration % 2000 == 0 and iteration > resume_iteration) or (iteration == 0):
            ...
            logging.info('Validate bal eer: {:.3f}'.format(
                np.mean(bal_statistics['eer'])))

            logging.info('Validate test mAP: {:.3f}'.format(
                np.mean(test_statistics['average_precision'])))

            logging.info('Validate test eer: {:.3f}'.format(
                np.mean(test_statistics['eer'])))


            # e = open("cnntrans-eer-test1-train_demucs-lre6-cat.txt", 'a')
            e = open("trans-eer-test3-train_demucs.txt", 'a')
            e.write('iteration:{}   '.format(iteration))
            e.write('Validate bal eer: {:.4f} '.format(
                np.mean(bal_statistics['eer'])))
            e.write('Validate test eer: {:.4f}\n'.format(
                np.mean(test_statistics['eer'])))
...
if __name__ == '__main__':
    ...
    parser_train.add_argument('--data_type', type=str, default='balanced_train', choices=['balanced_train', 'full_train'])
